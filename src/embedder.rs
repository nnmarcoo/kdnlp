...
pub fn load(model_dir: &Path) -> bool {
    MODEL.get_or_init(|| {
        ...
        let stats_text = match std::fs::read_to_string(&stats_path) {
            Ok(t) => t,
            Err(e) => { eprintln!("embedder: can't read {:?}: {e}", stats_path); return None; }
        };
        ...
        let exe_lib = std::env::current_exe().ok()
        ...
        let mut builder = match Session::builder() {
            Ok(b) => b,
            Err(e) => { eprintln!("embedder: session builder failed: {e}"); return None; }
        };
        let session = match builder.commit_from_file(&onnx_path) {
            Ok(s) => s,
            Err(e) => { eprintln!("embedder: ONNX load failed: {e}"); return None; }
        };

        eprintln!("embedder: loaded OK");
        Some(Model { session: Mutex::new(session), means, stds })
    }).is_some()
}
...
pub fn embed(typing: &TypingSession) -> Option<[f32; EMBED_DIM]> {
    ...
    let length = seq.len().min(MAX_LEN);

    // Build (1, MAX_LEN, 3) float32 tensor, zero-padded, z-normalized
    let mut data = vec![0f32; MAX_LEN * 3];
    for (i, &(_, iki, dwell, flight)) in seq.iter().take(MAX_LEN).enumerate() {
        let raw = [iki as f32, dwell as f32, flight as f32];
        for (j, (v, (&mean, &std))) in raw.iter().zip(model.means.iter().zip(model.stds.iter())).enumerate() {
            data[i * 3 + j] = (v - mean) / std;
        }
    }

    let x_tensor = Tensor::<f32>::from_array(([1usize, MAX_LEN, 3], data)).ok()?;
